...
They are kept together because the notebooks use working-directory-relative
...
**How this was verified.** Before any extraction, every function that looked
duplicated across the original notebooks was diffed programmatically (not by
eye) to confirm it really was byte-identical; only confirmed-identical code
was merged. After extraction, the class methods were checked against the
...
themselves cannot be re-run in an offline environment, and are not
...
Both work with the same SNAr kinetic simulator concept, but a diff performed
...
Renamed from the original numbered research-log notebooks ("dia" = Spanish
...
- **`buchwald_hartwig_core_grid.ipynb`** — the 10-seed × 30-iteration core
  ...
  notebook (dead code from the moment it was written); that first copy is not
  reproduced here.

- **`kernel_prior_bh.ipynb`** — KernelPrior / KernelPrior-Loop /
  BatchSelect-RAG vs. GP-BO, 20 seeds × 15 iterations. **Discovered
  ...
  notebook's own markdown cell for detail. This was *not* "fixed" here,
  ...
  — none of which is reproduced here. It also had four near-duplicate
  sibling notebooks (`dia11_bo_kernel_shaped*.ipynb`,
  `dia11_batch_llm_grounded_qwen30b*.ipynb`) that split this same run by
  method or by seed range purely for HPC scheduling; those have been removed
  as redundant with this single notebook, per the explicit request that
...
- **`ucb_vs_ei_bh.ipynb`** — adds the UCB acquisition arm (`bo_puro_ucb`,
  ...
  this notebook's own local run (10 seeds × 30 iterations) writes
  ...
  *different, smaller* file from `all_results_full_grid_20seeds.csv` (n=20),
  ...
  n=20 file is not reproducible from the code as currently saved. Both files
  are shipped in this directory regardless.

- **`direct_arylation_benchmark.ipynb`** — GP-BO / PCV / BatchSelect, EI and
  UCB, 20 seeds × 20 iterations. Fetches its dataset directly over HTTP from
  ...
  for provenance but are not needed to reproduce `results_n20_arylation.pkl`.
...
- **`snar_safety_multiagent.ipynb`** — the paper's safety-constrained case
  ...
  Agent), 20 seeds × 15 iterations, `qwen2.5:7b`. This is `v2` of the
...
- **`model_scale_analysis.ipynb`** — pure analysis (no LLM calls, no BO):
  ...
  numbering, different content, do not confuse the two.
...
broken, non-reproducible state and never wrote either file. The most likely
explanation is that they were produced by an earlier iteration of that
notebook that was subsequently edited in place without preserving that
version. **These two files are still included here** (they are required
inputs to `scripts/generate_figures.py`, and the validation in
`VALIDATION.md` confirms they reproduce the paper's exact reported numbers)
— only the notebook-level record of exactly how they were originally
...
notebook top-to-bottom is not guaranteed to regenerate every file
